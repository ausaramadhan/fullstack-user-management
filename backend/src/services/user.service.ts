...
export const getAllUsers = async (query: any) => {
  ...
  const result = {
    data: users,
    metadata: { totalData: total, totalPage: Math.ceil(total / limit), currentPage: page, perPage: limit }
  };
...
export const deleteUser = async (id: number, adminPass: string, actorId: number) => {
  ...
  if (!admin || !bcrypt.compareSync(adminPass, admin.password_hash)) throw new Error('Password admin salah');
...
export const getUserById = async (id: number) => {
  ...
  const user = await prisma.user.findUnique({
    where: { id },
    select: { id: true, name: true, username: true, role: true, created_at: true, updated_at: true }
  });
